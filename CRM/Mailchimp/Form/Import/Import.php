...
class CRM_Mailchimp_Selector_Import extends CRM_Core_Selector_Base implements CRM_Core_Selector_API {
    /**
     * @var array
     */
    private $importData;
    ...
    public function __construct($importId, $params = array()) {
        ...
        $this->bao = CRM_Mailchimp_BAO_Import_Source::factory($this->importData['source']);
    ...
    function &getRows( $action, $offset, $rowCount, $sort, $type = null ) {
        ...
        while($dao->fetch()) {
            ...
            $status = $row['status'];
            $row['error_status'] = CRM_Mailchimp_BAO_Import_SourceAbstract::isErrorStatus($status);
